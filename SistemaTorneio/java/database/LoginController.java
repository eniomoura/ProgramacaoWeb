...
public class LoginController extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			if (op.equals("entrar")) {
				if (AdminDAO.consultar(usuario, senha)) {
					//Obter a sessão.
			...
			} else if (op.equals("dbeditor")){
				req.getRequestDispatcher("dbclient.jsp").forward(req, resp);
			} else if (op.equals("cadastro")){
				resp.sendRedirect("/database/cadastro");
			}else if (op.equals("")) {
				msg = "";
			} else {

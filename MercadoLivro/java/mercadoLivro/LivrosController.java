...
public class LivrosController extends HttpServlet{		
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
			...
			String removerID= valor(req, "removerID", "");
			...
			if(removerID!=""){
				carrinho = (ArrayList<ItemCarrinho>) session.getAttribute("carrinho");
				ItemCarrinho itemRemove=null;
				for(ItemCarrinho i : carrinho){
					if(i.getID()==Integer.parseInt(removerID)){
						itemRemove=i;
					}
				}
			...
			req.setAttribute("itemCode", geradorListaLivros(CarrinhoDAO.listar()));
	...
	public String geradorListaLivros(List<ItemCarrinho> items){
		DecimalFormat df = new DecimalFormat();
		df.setMinimumFractionDigits(2);
		String itemCode="";
		for(ItemCarrinho item : items){
			if(item.getTipo().equals("livro")){
...
"											");
	...
	public String gerarCarrinhoCode(HttpSession session){
		...
		float valorTotal=0;
		if(session.getAttribute("logged").equals("true")&&carrinho!=null&&carrinho.size()>0){
			for(ItemCarrinho item : carrinho){
				carrinhoCode=carrinhoCode+"<li class=\"list-group-item\">"+ item.getNome() + "<br><a href=\"?removerID="+ item.getID() +"\" style=\"color:red\" class=\"pull-right\">[-]</a>" +"<br><span class=\"badge\"><sup>R$</sup> "+ df.format(item.getPreco()) +"</span><span class=\"badge\">"+ item.getQuantidade() +"x</span><br></li>";
				valorTotal+=item.getPreco()*item.getQuantidade();
			}
			carrinhoCode=carrinhoCode+"<li class=\"list-group-item\">Valor total: <sup>R$</sup> "+ df.format(valorTotal) +"<span class=\"badge\"></span></li><li class=\"list-group-item\"><form><button name=op value=checkout>Completar Compra</button></form></li></ul>";
		}else{
			carrinhoCode="</ul>";
		}
		return carrinhoCode;
